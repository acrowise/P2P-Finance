  	...
  	methods: {	
	    ...
	    href:function(state,index){
	    	...
	    	if(state!=2){
	    		 ...
	    		 sui.open('payCalendarDetail.html','payCalendarDetail.html',{d:d},{},true,"回款日历",false);
	    	}
	    },
	    init:function(receiptDate,upOrDown){
	    	var w=sui.wait();
	    	sui.request('Account/ReceiptCalendar',{receiptDate:receiptDate,upOrDown:upOrDown},true,function(data){
	    		if(data){
	    			if(data.IsPass){
