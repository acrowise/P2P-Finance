var vm = new Vue({
	...
	methods: {
		init: function() {
			setTimeout(function() {
				sui.request('User/SetPayPassword', {tradeType:1}, true, function(data) {
					if(data) {
						if(data.IsPass) {
							vm.plusReady(data.ReturnObject);
						} else {
							if(data.StatusCode=='S0053'){
		...
		plusReady: function(data) {
			...
			this.embed.overrideUrlLoading({
				mode: "reject",
				match: ".*#/bank/businessAuth.*"
			}, function(e) {
				 sui.open('businessAuth.html','businessAuth.html',{},{},true,'业务授权',false,true,"\ue705",function(){
		             	 sui.open('authExplain.html','authExplain.html',{},{},true,'授权说明',false);
		             });
				setTimeout(function(){
